...
export class TestListRenderer extends ListRenderer {
    ...
    toggleRecordSelection(record) {
        console.log("toggleRecordSelection 2.0")
        this.props.list.unselectRecord(true);

        // if (!this.canSelectRecord) {
        //     console.log("toggleRecordSelection 2.1 ", this.canSelectRecord)
        //     return;
        // }
    ...
    get hasSelectors() {
        ...
        if (
            (dataRecord.state === "jefatura" &&
                dataRecord.uid_create === 3 &&
                dataRecord.current_user === 1) ||
            (dataRecord.state === "contable" && dataRecord.uid_create === 2)
        ) {
    ...
    async _loadPro() {
        ...
        if (
            await session.user_has_group("gastos_tqc.res_groups_aprobador_gastos")
        ) {
            this.showRemoveIcon.value = false;
        }
    }
            ...
            await rpc.query({
                ...
                args: [
                    selected_list,
                    {
                        ...
                        [state === "contable"
                            ? "observacioncontabilidad"
                            : "observacionjefatura"]: this.descriptionObserva.text,
                    },
                ],
            });
